...
| SRC-02 | Treat hierarchy/names/role intent as authoritative; do not infer hidden integrations/duplicate reasons. | 00, 02 | Source references, confidence, implementation notes | Catalog source-note/duplicate tests | planned |
...
| ID | Requirement | Plan | Planned implementation | Verification | Status |
|---|---|---|---|---|---|
| ARCH-01 | Use Python 3.12/FastAPI/Pydantic/SQLAlchemy/Alembic when greenfield. | 01 | Backend workspace | Toolchain/build checks | planned |
...
| DOM-02 | Every template has ID/name/department/function/purpose/instructions. | 02 | Template schema/files | Completeness test | planned |
...
| SAFE-10 | Never commit secrets; safe `.env.example`. | 01, 13, 15 | Config/ignore/CI scan | Secret scan/settings tests | planned |
| SAFE-11 | Tests never call real providers/services. | 10, 14 | Mocks/network blockers | Offline/no-network gates | planned |
...
| DEL-08 | Format/lint/type/test task runner commands. | 01, 14, 15 | Makefile/configs | Static gates | planned |
...
|---|---|---|---|
| EXEC-01 | Inspect references/guidance. | Plan 00 evidence inventory | planned |
