...
class ApprovalDecisionService:
    ...
    async def decide(
        ...
        *,
        ...
        self._validate_command_and_principal(command, principal)
        async with self._dependencies.unit_of_work() as unit_of_work:
            result = await self.decide_in_uow(
                unit_of_work,
                command,
                principal=principal,
            )
            parent = await unit_of_work.runs.get(result.action.run_id)
            if parent is None:
                raise ApprovalDecisionServiceError(
                    "approval_run_missing",
                    "approval decision parent Run does not exist",
                )
            if parent.state is RunState.AWAITING_APPROVAL and parent.approval_required is True:
                # Import locally to keep the decision primitive reusable by the
                # boundary composer without creating a module cycle.
                from .approval_boundaries import ApprovalBoundaryService
                ...
                await ApprovalBoundaryService(self._dependencies).evaluate_in_uow(
                    unit_of_work,
                    result.action.run_id,
                    audit_context=audit_context,
                )
            elif parent.state is not RunState.RECEIVED:
                raise ApprovalDecisionServiceError(
                    "approval_run_not_actionable",
                    "approval decision parent Run is outside the composed boundary state",
                )
    ...
    async def decide_in_uow(
        ...
        *,
        ...
        """Persist one decision without committing so the boundary can compose atomically."""
        ...
        try:
            selection = await unit_of_work.approvals.get_current_authorization_set(request.run_id)
        except ApprovalRepositoryConflict:
            raise ApprovalDecisionServiceError(
                "approval_record_corrupt",
                "authorization set head could not be validated",
            ) from None
        if (
            selection is None
            or selection.authorization_set.status is not AuthorizationSetStatus.OPEN
            or selection.authorization_set.id != request.authorization_set_id
            or selection.authorization_set.plan_hash != request.plan_hash
            or selection.authorization_set.proposal_revision != request.proposal_revision
        ):
            raise ApprovalDecisionServiceError(
                "approval_generation_conflict",
                "approval request is outside the current open authorization set",
            )
    ...
    @staticmethod
    def _validate_command_and_principal(
        command: ApprovalDecisionCommand,
        principal: AuthenticatedPrincipal,
    ) -> None:
