"""Atomic planner-owned dry-run proposal output; no connector authority or calls."""
...
from marketing_agents.domain.enums import Effect, RunState, StepState, WorkMode
...
class ProposalPreviewRenderer(Protocol):
    def render_proposal(
        self, template_id: str, admitted_input: Mapping[str, Any]
    ) -> Mapping[str, Any]: ...


class ProposalPreviewExecutionError(RuntimeError):
...
class ProposalPreviewExecutor:
    """Persist one proposal document and its no-call step/audit atomically.
    ...
    def __init__(
        ...
        renderer: ProposalPreviewRenderer,
    ) -> None:
    ...
    async def execute(self, step_id: str, *, audit_context: AuditContext) -> ArtifactEnvelope:
        ...
        async with self._dependencies.unit_of_work() as uow:
            ...
            if run is None or plan is None or work is None or control is None:
                raise ProposalPreviewExecutionError("preview_execution_snapshot_missing")
            ...
            if (
                definition.execution_kind is not CatalogRoleExecutionKind.WRITE
                or definition.handler_kind is not ExecutableWorkflowHandler.CATALOG_WRITE
                or definition.capability_id not in PROPOSAL_PREVIEW_CAPABILITIES
                ...
                raise ProposalPreviewExecutionError("preview_execution_contract_invalid")
            artifact_id = "artifact.preview." + hashlib.sha256(step.id.encode("utf-8")).hexdigest()
            ...
            if step.state is StepState.SUCCEEDED:
                if existing is None or existing.provenance.step_id != step.id:
                    raise ProposalPreviewExecutionError("preview_output_replay_invalid")
                return existing
            if existing is not None or step.state is not StepState.READY:
                raise ProposalPreviewExecutionError("preview_step_not_ready")
            ...
            if (
                ...
                raise ProposalPreviewExecutionError("preview_execution_fence_lost")
            ...
            if canonical_payload_size_bytes(
                ...
                raise ProposalPreviewExecutionError("preview_input_budget_exceeded")
            compile_json_schema(
            ...
            payload = json.loads(
                canonical_json_bytes(
                    self._renderer.render_proposal(step.template_id, work.admitted_payload)
                )
            )
            ...
            if (
                ...
                raise ProposalPreviewExecutionError("preview_execution_deadline_exceeded")
            if canonical_payload_size_bytes(payload) > budget.max_output_bytes:
                raise ProposalPreviewExecutionError("preview_output_budget_exceeded")
            compile_json_schema(
            ...
            for result in (started, finished):
                if not await uow.run_steps.apply_transition(
                    ...
                    raise ProposalPreviewExecutionError("preview_step_fence_lost")
            ...
            await uow.audits.append_many(
                (
                    ...
                    factory.artifact_previewed(artifact, finished.step),
                )
            )
            await uow.commit()
            return artifact
