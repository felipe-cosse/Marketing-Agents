// DEMO-04 unit evidence invokes focused transport and journey tests without Corepack or HOME state.
...
function runLocalTool(name, args) {
  ...
  if (!existsSync(executable)) {
    process.stderr.write(
      `DEMO-04 unit evidence requires installed ${name} from make web-bootstrap.\n`,
    );
    process.exit(2);
  }
  ...
  if (result.error !== undefined) {
    process.stderr.write(
      `DEMO-04 unit evidence could not start ${name}: ${result.error.message}\n`,
    );
    process.exit(2);
  }
  if (result.signal !== null) {
    process.stderr.write(
      `DEMO-04 unit evidence ${name} stopped by signal ${result.signal}.\n`,
    );
    process.exit(2);
  }
